...
type Props = {
  ...
  pageJump: string;
  setPageJump: React.Dispatch<React.SetStateAction<string>>;
  submitJump: () => void;
  ...
  return (
    <>
        ...
        <div className="toolbar-inner">
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M6 5v14M20 6l-9 6 9 6" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>

          <button
            className={`toolbtn ${!p.canPrev ? "disabled" : ""}`}
            onClick={p.goPrev}
            disabled={!p.canPrev}
            title="Previous"
          ...
          <button
            className={`toolbtn ${!p.canNext ? "disabled" : ""}`}
            onClick={p.goNext}
            disabled={!p.canNext}
            title="Next"
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M18 19V5M4 18l9-6-9-6" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>

          <button
            className={`toolbtn tool-zoom ${p.loupeOn ? "active" : ""}`}
            onClick={() => p.setLoupeOn((v) => !v)}
            aria-pressed={p.loupeOn}
            title={p.loupeOn ? "Disable magnifier" : "Enable magnifier"}
      ...
      <style jsx global>{`
        ...
        .portal-loupe {
          position: fixed; z-index: 60; border-radius: 999px; overflow: hidden;
          box-shadow: 0 10px 26px rgba(0,0,0,.24), inset 0 0 0 2px rgba(255,255,255,.9);
          pointer-events: none; background:#fff; contain: layout paint; will-change: transform; transform: translateZ(0);
        }
      `}</style>
    </>
  );

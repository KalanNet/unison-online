...
export default function MobilePager(p: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <MobileSwipe
  key={ctrl.currentIndex}                       // ⬅️ fix "залипання/відкату"
  onSwipeLeft={() => ctrl.canNext && ctrl.goNext()}
  onSwipeRight={() => ctrl.canPrev && ctrl.goPrev()}
  threshold={80}
>
  {(() => {
    // ----- простий зум: double-tap і pinch -----
    ...
    const onTap = () => {
      ...
      if (now - lastTapRef.current < 260) {
        setZoom((z) => (z === 1 ? 2 : 1));
        if (zoom !== 1) { setTx(0); setTy(0); }     // скинути панорамування при відзумі
      }
    ...
    const onPointerMove = (e: React.PointerEvent) => {
      ...
      if (pts.current.size === 2 && baseDist.current) {
        ...
        // панорамування при зумі (одним пальцем)
    ...
    // обмежити панорамування, щоб сторінка не «тікала»
    const maxPan = 180; // px, м’яка межа

...
export default function AdRail({ open, onToggle }: Props) {
  ...
  .ad-rail {
    position: fixed;
    /* прив’язуємо до меж робочої області між хедером та футером */
    top: var(--hdr);
    bottom: var(--ftr);
    left: 12px;
    width: 240px;
    transition: transform 260ms ease, opacity 260ms ease;
    z-index: 350;           /* поверх усього контенту в сцені */
    pointer-events: auto;   /* кліки завжди активні */
  }

  /* коли ЗАКРИТО — заганяємо за край, лишаємо «вушко» */
  .ad-rail.is-closed {
    transform: translateX(calc(-100% + 32px));
  }

  /* коли ВІДКРИТО — жодних трансформацій */
  .ad-rail.is-open {
    transform: none;
  }
